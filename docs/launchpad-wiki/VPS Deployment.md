...
- [ ] **Remove `11434:11434` host port mapping from `ollama` service** in `docker-compose.yml` so Ollama is only reachable inside the `traefik` network (mirrors how Postgres is handled). See [[AI Integration]] follow-up security item.

### Frontend (separate track)
- [ ] Update frontend spec to include the two AI endpoints (`POST /api/ai/ask`, `GET /api/ai/logs/analyze`) so the dashboard can surface log analysis per app.
...
- [ ] Smoke-test [[AI Integration]] on VPS (`/api/ai/ask` + `/api/ai/logs/analyze` against a real deployed app) after pulling the model on the VPS with `docker exec -it ollama ollama pull llama3.2:3b`
